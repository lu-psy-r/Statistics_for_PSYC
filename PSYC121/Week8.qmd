...
write_csv(d, file = "data/Week_8/stroop.csv")
...
1.  Create a folder for Week 8 and download the [Week_8 csv file](data/Week_8/stroop.csv) file and upload it into this new folder in RStudio Server.
...
7.  Let's use the filter command we learned last week to remove these high values. Like last week, we will do this (for now) in a fairly unprincipled manner, by "eyeballing" the data (next week we'll consider something a bit more "scientific"). Complete the filter command so that it removes the highest values. It's your choice where you set this limit. Add the code below and write an expression to filter out the high values (remember that filter KEEPS the rows which are TRUE for the expression). Also, remember that you need to think about how you are storing the result of this filter process. Do you want to create a new object, or overwrite the existing object? MAke sure you edit the final bit of code to plot a histogram of the *filtered* data.
...
8. Finally, copy the code for the original `geom_density()` plot that you drew in step 4. Paste it, and edit the code so that it now plots the filtered set of data (from step 7) for each of the three conditions in the stroop task. 
...
6.  With 3 levels to the IV *condition* there are 3 possible comparisons we can make (L1 vs. L2; L1 vs. L3; L2 vs. L3). Complete all three tests, by copying the code chunk, editing it to make a different filter selection, and then running the t-test. Write out a reporting statement (Q5) for each of your comparisons.
...
- Are there any red cross symbols next to your lines of code? These inidcate a code error and need to be fixed before it will knit. 
