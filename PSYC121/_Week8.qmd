...
data_proc <- 
  ...
  pivot_longer(cols = contains("stroop"),
               ...
               values_to = "time") %>% 
  mutate(time = round(as.numeric(time),1)) %>% 
  drop_na()

write_csv(data_proc, file = "data/Week_7/stroop.csv")

data_proc %>%
  group_by(condition) %>% 
  summarise(mean_time = mean(time, na.rm = TRUE)) %>% 
  ggplot(aes(y = mean_time, x = condition, fill = condition)) +
    geom_col()
...
1.  Open the script "Week_8\_script.R" (see prep work)
