...
3. Take a look at the summary statistics for all of the columns in our data using `summary(your_data_object_name)`
...
### Task 2 - Visualising relationships in the data
...
6.  Add a filter command to remove the values in the *z_imm* column are greater than 2.5 or less than -2.5. That means you'll need two seperate statements with an & in the middle
...
7. You should have **removed 1 row of data**. Make a note of this in your R markdown document. 
...
# run unrelated samples t-test
t.test(pop_est_immigrants ~ home_location_in_UK, data = data_wk9_f, 
       paired = FALSE, 
       var.equal = TRUE) # you'll need to set this to TRUE or FALSE depending on what you found in var.test

```

5. If you've done this correctly, you should have a **t-statistic = -2.0735**. Examine the p-value and discuss on your table what this means. Write a line in your .Rmd to formally state the result of the t-test (look at last week for the format). Describe what this result means in your own words.
