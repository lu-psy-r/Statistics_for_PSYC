...
3.  At this stage you could try using `ggplot()` with `geom_histogram()` or `geom_density()` to explore the different columns. You did this last week, so take a look at the instructions from last week or your .Rmd from last week to remind yourself. 
