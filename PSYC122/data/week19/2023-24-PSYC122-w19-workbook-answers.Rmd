...
But, if you compare the correlation estimates, you can see that the coefficient estimate is smaller in `study.122` (where r = .32) than in `study.two.gen` (where r = .40).
...
- A.23. It is significant, p > .05 because p = 1.52e-05
...
- A.26. If you compare the linear model coefficient estimates from both the `study.two.gen` and `study.122` models, you can see:
...
- Q.28. Given the information in the adjusted plots, can you explain what is different about the `HLVA` effect estimate in the `study.122` data compared to the ?
