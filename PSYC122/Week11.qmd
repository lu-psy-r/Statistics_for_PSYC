...
<!-- (data/week11/122_week11_forStudents.zip) -->
...
Further lab activities will be added very soon.
...
0.  Start a new RMarkdown script. If you need a reminder of how to do that, please revisit week 6 of PSYC121 ([here](https://lu-psy-r.github.io/statistics_for_psychologists/PSYC121/Week6.html#some-basic-rmarkdown)).

1.  Add the code to load the 'broom' and the 'tidyverse' libraries in a new code chunk. If you are unsure, you can look at the 'Hint' below for a clue by expanding it. After that, if you are still unsure, you can view the code by expanding the 'Code' section below. 
...
3.  Read in the data. You should now see a dataframe with 25 observations and 5 variables in the 'Environment'. Click on it to view it.
...
4.  Construct a scatterplot of the relationship between 'Home' and 'TV'. Also add a line of best fit.
...
::: {.callout-tip collapse="true"}
## Hint 2
Below is a template of the code, make sure to add the dataframe and the relevant variables names. Also add clear labels for your axes.
```
ggplot(DATA, aes(x = , y = )) + 
  geom_point() +
...
5.  What can you tell from the scatterplot about the direction of the relationship? Write a few sentences in your R Markdown script to describe the relationship.

6.  Conduct the correlation analysis.
::: {.callout-tip collapse="true"}
## Hint 1
Use the `ggplot()`function in combination with `geom_point()` and `geom_smooth()`. Make sure you use clear labels for your axes by using `labs()`.
:::
...
::: {.callout-tip collapse="true"}
## Hint 2
Below is a template of the code, make sure to add the dataframe and the relevant variables names.
```
...
7.  What is the correlation coefficient (Pearson's r)?

8.  What is the p value?

9.  Is the correlation significant at the p \< .05 level?

10.  What are the degrees of freedom you need to report?

11. How much variance in 'time spent reading' can be accounted for by 'time spent watching TV'? (Hint: you can use the Console in RStudio as a calculator.)

12. Write a few sentences in which you report this result, following APA guidelines.
